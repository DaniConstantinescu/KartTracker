...
public class newCircuitController {

    @FXML
    TextField inputNume, inputBest, inputPers, inputTara, inputTurns, inputLungime;
    ...
    @FXML
    protected void addCircuit(ActionEvent event) throws IOException{


        if ( inputNume.getText() .equals("") || inputBest.getText() .equals("") || inputPers.getText() .equals("") || inputTara.getText() .equals("") || inputTurns.getText() .equals("") || inputLungime.getText() .equals("") ){
            ...
            String[] tokens= inputBest.getText().split(":");
            Timp best = new Timp( Integer.parseInt(tokens[0]), Integer.parseInt(tokens[1]), Integer.parseInt(tokens[2]) );

            tokens= inputPers.getText().split(":");
            Timp pers = new Timp( Integer.parseInt(tokens[0]), Integer.parseInt(tokens[1]), Integer.parseInt(tokens[2]) );


            DbUtilities.inserareCircuitDB( inputNume.getText(), Timp.convertTimpToMs(best), Timp.convertTimpToMs(pers), inputTara.getText(), Integer.parseInt(inputTurns.getText()) , Integer.parseInt(inputLungime.getText()) );
